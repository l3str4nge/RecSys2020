...
import common.core.utils.MLConcurrentUtils.Async;
...
public final class MLXGBoost {
	...
	private static final LabeledPoint EMPTY_LABELED_POINT = new LabeledPoint(0, new int[0], new float[0]);
	...
	public static DMatrix toDMatrix(final MLSparseMatrix matrix)
		...
		Iterator<LabeledPoint> it = new Iterator<LabeledPoint>() {
			...
			public LabeledPoint next() {
				...
				cur += 1;
				if (row == null) {
					return EMPTY_LABELED_POINT;
				} else {
					return new LabeledPoint(0, row.getIndexes(), row.getValues());
				}
			}
		};
