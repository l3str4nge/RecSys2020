...
def train(args, data_generator, model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    """ Train the model """
...
#
    ...
    if args.continue_training:
        model.load_state_dict(torch.load(args.continue_training_path + "model.bin"))
        optimizer.load_state_dict(torch.load(args.continue_training_path + "optimizer.pt"))
        scheduler.load_state_dict(torch.load(args.continue_training_path + "scheduler.pt"))
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            tokens, features, targets, inputs, attention_mask, lens = [x.to(args.device) for x in [
                tokens, features, targets, inputs, attention_mask, lens
            ]]

            tokens, features, targets = [x.float() for x in [tokens, features, targets]]
...
def evaluate(args, data_generator, tb_writer, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, global_step, prefix="") -> Dict:
    ...
    for i, engage in enumerate(["reply", "retweet", "comment", "like"]):
        ...
        tb_writer.add_scalar('PRAUC/{}_val'.format(engage), _prauc, global_step)
...
def main():
    ...
    parser.add_argument("--continue_training_path", default="../checkpoints/supervised_difflr/checkpoint-56000/")
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, data_generator, mlp_model, tokenizer)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, global_step=0)
