...
class Data:
    def __init__(self, dpath, tr_name, val_name, u_threshold=7, c_threshold=7,is_lb=False):
        if not is_lb:
            ...
            #_cut_by_counts
            if u_threshold > 0:
                unique_uId = self._cut_by_counts(tr_engages, 'userId', u_threshold)         
            if c_threshold > 0:
                unique_cId = self._cut_by_counts(tr_engages, 'creatorId', c_threshold)
            ...
            s = [uId2trans_uId, cId2trans_cId]
            ##Save scalers
            with open('./data/cf.pkl', 'wb') as f:
