...
def get_mentions(df, col_name):
    ...
    df["tweet_clean"] = df[col_name].str.replace(main_mention_reg, "<mention>")
    df["other_mentions"] = df["tweet_clean"].str.extractall(other_mentions_reg).groupby(level=0)["item"].apply(list)
    df["tweet_clean"] = df["tweet_clean"].str.replace(other_mentions_reg, "<mention>")
...
def get_urls(df, col_name):
    ...
    df["tweet_clean"] = df[col_name].str.replace(url_reg, "<url>")
...
def process_chunk(lines, chunk_num, mode):

    print("Processing chunk {}...".format(chunk_num))
    
    if mode == "train":
        df = pd.DataFrame(lines, columns=all_features+labels_to_idx)
    else:
    ...
    # save
    df.to_pickle("/home/layer6/recsys/clean/xlmr/chunk{}.p".format(chunk_num), protocol=pickle.HIGHEST_PROTOCOL)
    # df.to_csv("/media/kevin/datahdd/data/recsys/tweetstring/train/chunk{}.tsv".format(chunk_num), sep='\t')
    # df.to_pickle("/media/kevin/datahdd/data/recsys/tweetstring/train/test.p", protocol=pickle.HIGHEST_PROTOCOL)

    print("Done processing chunk {}, time elapsed: {}".format(chunk_num, (time.time()-start)//60))


def main(mode):
    
    file_path = "/home/layer6/recsys/raw/training.tsv"
    # file_path = "/media/kevin/datahdd/data/recsys/raw/training.tsv"
    ...
    with open(file_path, encoding="utf-8") as f:
        ...
        while True:
            ...
            chunk_lines = list(islice(f, 15000000))
            chunk_lines = [x.strip().split("\x01") for x in chunk_lines]

            process_chunk(chunk_lines, chunk_number, mode)
            chunk_number += 1
